...
import { Card, DemoAssetImage, SectionHeading } from "./primitives";
import { Section } from "./section-primitives";
import type { ServicesVariant } from "@/lib/demo/types";

// Three deliberately distinct presentations chosen by the Site Director
// based on service count/density/cardPolicy — "feature-panels" specifically
// fixes V2.1's problem of stretching 1-2 confirmed services across a grid
// built for six; "editorial-rows" is the card-free option for
// cardPolicy: minimal families.
export function Services({ ctx, variant }: { ctx: DemoRenderContext; variant: ServicesVariant }) {
  ...
  const width = sectionWidth.services ?? "contained";

  return (
    <Section id="services" spacing={spacingScale} width={width}>
      <SectionHeading eyebrow="What we do" heading="Services" subheading={ctx.copy.services_intro} align={align} />
          ...
          {cards.map((c, i) => (
            <div key={c.name} className={`group flex flex-col gap-8 md:flex-row md:items-center ${i % 2 === 1 ? "md:flex-row-reverse" : ""}`}>
              ...
              <div className="md:w-1/2">
                <h3 style={{ fontFamily: "var(--demo-font-heading)" }} className="text-3xl font-medium text-[var(--demo-text)]">
                  {c.name}
                </h3>
          ...
          {cards.map((c, i) => (
            <div key={c.name} className="group relative overflow-hidden rounded-[var(--demo-radius)] shadow-lg">
              ...
              <div className="absolute inset-x-0 bottom-0 p-7">
                <h3 style={{ fontFamily: "var(--demo-font-heading)" }} className="text-2xl font-medium text-white">
                  {c.name}
                </h3>
        ...
        <div className="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
          {cards.map((c, i) => (
            <Card key={c.name} className="overflow-hidden p-0">
              ...
              <div className="p-6">
                <h3 style={{ fontFamily: "var(--demo-font-heading)" }} className="text-lg font-medium text-[var(--demo-text)]">
                  {c.name}
                </h3>
