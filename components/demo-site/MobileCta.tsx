...
export function MobileCta({ profile }: { profile: DemoBusinessProfile }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-40 flex border-t border-[var(--demo-border)] bg-[var(--demo-bg)] shadow-[0_-2px_10px_rgba(0,0,0,0.08)] md:hidden">
      <a href={phoneHref(phone)} className="flex-1 py-3.5 text-center text-sm font-semibold text-[var(--demo-text)]">
        Call
      </a>
      <a href="#contact" className="flex-1 bg-[var(--demo-accent)] py-3.5 text-center text-sm font-semibold text-[var(--demo-accent-text)]">
        Get a Quote
      </a>
    </div>
  );
